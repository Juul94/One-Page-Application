...
const filterTodos = (userText) => {

    // Adding class "filtered" to the notes which doesn't contain the search value
    Array.from(list.children)
        .filter((todo) => !todo.textContent.toLowerCase().includes(userText))
...
searchForm.addEventListener('keyup', () => {
    ...
    const userText = searchForm.value.trim().toLowerCase();

    filterTodos(userText);
